...
def test_agent_routes_to_metadata_query():
    ...
    with (
        ...
        patch("text_to_sql.nodes.format_meta.default_llm", mock_meta_llm),
    ):
...
def test_agent_routes_to_ambiguous_query():
    ...
    with (
        patch("text_to_sql.nodes.classify.default_llm", mock_classifier_llm),
        patch("text_to_sql.nodes.clarify.default_llm", mock_clarify_llm),
    ):
        result = agent.invoke({"question": question}, config=config)
...
def test_agent_routes_to_out_of_scope_query():
    ...
    with patch("text_to_sql.nodes.classify.default_llm", mock_classifier_llm):
        result = agent.invoke({"question": question}, config=config)
